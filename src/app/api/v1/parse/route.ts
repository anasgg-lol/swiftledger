...
const CONCURRENCY = 8; // Parallel model tasks in flight simultaneously
...
function tryParseLine(line: string): { date: string; description: string; amountToken: MoneyToken; balanceToken: MoneyToken } | null {
  ...
  const dateEnd = dateMatch[0].length; // Explicitly pull first match string bounds index length
...
export async function POST(req: Request) {
  try {
    ...
    const dynamicPdfParse = require('pdf-parse'); // Modern dynamic scoping for Vercel compliance

    // Compile text layers up front cleanly without loop leaks
    ...
    // Concurrent multi-threaded async network parallel execution layers
